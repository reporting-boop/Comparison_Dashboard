    ...
    <div ref={ref} style={{ position:"relative" }}>
      ...
      {open && (
        <div style={{
          ...
          border:"1px solid #e5e7eb", minWidth:180, maxHeight:260, overflowY:"auto",
          padding:"6px 0",
        }}>
          {/* Clear all */}
          {selected.length > 0 && (
            <div
              onClick={() => onChange([])}
              style={{
                padding:"7px 14px", fontSize:11, color:PINK,
...
export default function MomPage({ storeData, marketData, districtData, user }) {
  ...
  const [markets, setMarkets] = useState([]);   // array now
  ...
  const winLose = useMemo(() => {
    return ["ppd","acc","voice","bts","hint","upgrades","retention"].map(m => {
      ...
      return {
        ...
        up:   filteredStores.filter(r => (r[pctKey]||0) > 0).length,
        down: filteredStores.filter(r => (r[pctKey]||0) < 0).length,
      };
    });
  }, [filteredStores]);
        ...
        <div style={{ background:"#fff", borderRadius:14, border:"1px solid #e9eaf0", padding:"18px 20px", marginBottom:20 }}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={chartData} barCategoryGap="25%">
              ...
              <Bar dataKey="prev" name="Previous" fill="#52796f" radius={[3,3,0,0]}/>
            </BarChart>
          </ResponsiveContainer>
        </div>
